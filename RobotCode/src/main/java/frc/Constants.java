...
public class Constants {
    ...
    static public final boolean[] wheel_direction() {
        boolean[] INVERT_WHEEL_DIRECTION_ISREAL = {false, false, false, false};
        boolean[] INVERT_WHEEL_DIRECTION_ISSIM =  {true, false, true, false};
        if (Robot.isReal()) {
    ...
    static public final double[] get_azimuth_encoder_mount_offset_rad() {
        double[] AZMTH_ENCODER_MOUNT_OFFSET_RAD_ISREAL = {
            Units.degreesToRadians(4.0-11.95-90.0), //added "4.0-" to beginning
            Units.degreesToRadians(41.89-90.0+180.0),
            Units.degreesToRadians(-1+35.5-90.0),
            Units.degreesToRadians(-3-10.5-90.0)};
        double[] AZMTH_ENCODER_MOUNT_OFFSET_RAD_ISSIM = {-2.157, -1.575, -2.180, -0.803};
        if (Robot.isReal()) {
    ...
    //YN: later on, we may discover that we can simply set everything to 0, but until then adding in configurability.
    static public final double get_gyro_offset_rad() {
        if (Robot.isReal()) {
            return Units.degreesToRadians(180.0); //negated from original 90.0, next was -90 (0 is forward, 180 is inverted)
        } else {
            return 0.0;
        }
    }
    ...
    static public final boolean[] finger_direction() {
        boolean[] INVERT_FINGER_DIRECTION_ISREAL = {false, true};
        boolean[] INVERT_FINGER_DIRECTION_ISSIM =  {false, true};
        if (Robot.isReal()) {
